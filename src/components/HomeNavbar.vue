            ...
            <ul class="navbar-nav flex-row px-3 col-12 col-md-6 justify-content-end">
              <li class="nav-item d-none d-md-block">
                <div class="btn-group">
                  ...
                  >
                    <h6 class="fas fa-shopping-cart mb-0 cart-icon">
                      <div
                        class="cartQty-box bg-danger d-flex justify-content-center align-items-center"
                        v-if="cartLen > 0"
                      >
                        <div class="cartQty text-white text-center">{{ cartLen }}</div>
                      </div>
                    </h6>
                  </a>
                  <div class="dropdown-menu dropdown-menu-right cart-dropdown p-0 border-0">
                    ...
                    <div class="row dropdown-footer p-3">
                      <div class="col-4 d-flex justify-content-center align-items-center">
                        總計:
                        <span
                          class="h5 text-primary mb-0 ml-2"
                        >{{ carts.final_total | currency }}</span>
                      </div>
                      <div class="col-8">
                        <router-link
                          to="/cartpage"
...
import $ from "jquery";
import navbarFixed from '../mixin/navbarFixed'
export default {
  data() {
    return {
      carts: [],
...
.homeNavbar {
  ...
  box-shadow: 0 0.5px 10px 0.5px #000;
}
.position-fixed {
  position: fixed;
}
